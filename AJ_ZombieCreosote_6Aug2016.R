...
# Savepoint!
write.csv(allzomdat, file="/Users/alesia/Documents/data_AJ_output/allzomdat26Sept2016.csv", row.names=F)
...
WigStemWP.cor.plot <- ggplot(unique(allregs[,c("timestamp", "SoilWaterPot", "mean.dif", "StemID", "LDStatus")]), aes(x=SoilWaterPot, y=mean.dif, group=StemID, colour=LDStatus)) + theme_bw() + theme(legend.position="none") +
  geom_point(alpha=0.3) + 
  stat_smooth(method="lm") + 
  #geom_text(data=raw.corr, aes(x=mean(allregs$StemWPmornsm,na.rm=T), y=ypos, label=StemWPest, size=0.2), colour="red", vjust=1, parse=TRUE, show.legend=F)
