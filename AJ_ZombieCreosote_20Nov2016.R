...
GPPfile <- aggregate(cbind(GPP, RE)~timestamp, data = GPPfile, sum, na.rm=TRUE, na.action="na.pass")
GPPfile$timestamp <- as.POSIXct(strptime(GPPfile$timestamp, format= "%Y-%m-%d %H:%M:%S"))
...
# Savepoint!
write.csv(allzomdat, file="/Users/alesia/Documents/data_AJ_output/allzomdat20Nov2016.csv", row.names=F)
...
stems.plot.mean <- ggplot(Wigglesub, aes(x=timestamp, y=mean.dif, group=StemID)) +
  ...
  geom_point(colour="#a65628", aes(y=Wigglesub$mean.dif.LD)) + geom_line(colour="#a65628", aes(y=Wigglesub$mean.dif.LD[Wigglesub$LDStatus=="D"])) +
  theme(legend.position="none") +
  ylab("Branch Position") + xlab("Time") +
...
VPD.plot <- ggplot(VPDsub, aes(x=timestamp, y=VPD)) +
  geom_point(colour="purple", alpha=0.5) + theme_bw() +
  theme(legend.position="none") +
  xlab("Time") + ylab("VPD") +
  xlim(start.date, end.date)
...
plot(allregs$GPP[allregs$LDStatus=="L" & month(allregs$timestamp) <= 9 & allregs$GPP > 0], allregs$mean.dif.LD[allregs$LDStatus=="L" & month(allregs$timestamp) <= 9 & allregs$GPP > 0])
...
plot(allregs$mean.dif.LD[allregs$LDStatus=="L"], allregs$GPP[allregs$LDStatus=="L"])
c(cor.test(allregs$GPP[allregs$LDStatus=="L"], allregs$mean.dif.LD[allregs$LDStatus=="L"])$estimate, 
  cor.test(allregs$GPP[allregs$LDStatus=="L"], allregs$mean.dif.LD[allregs$LDStatus=="L"])$p.value)
...
c(cor.test(allregs$VPD, allregs$mean.dif.all)$estimate, cor.test(allregs$VPD, allregs$mean.dif.all)$p.value)
c(cor.test(allregs$VPD[allregs$LDStatus=="L"], allregs$mean.dif.LD[allregs$LDStatus=="L"])$estimate, cor.test(allregs$VPD[allregs$LDStatus=="L"], allregs$mean.dif.LD[allregs$LDStatus=="L"])$p.value)
...
VPDoneweek <- ggplot(unique(allregs[,c("timestamp", "VPD")]), aes(x=timestamp, y=VPD)) +
  ...
  geom_line(alpha=0.5, colour="purple") + 
  theme(legend.position="none") +
  xlab("Time") + ylab("VPD") +
  xlim(tenday.start, tenday.end)

WigVPD.cor.plot <- ggplot(unique(allregs[,c("timestamp", "VPD", "mean.dif.all")]), aes(x=VPD, y=mean.dif.all)) + theme_bw() + theme(legend.position="none") +
  geom_point(aes(alpha=0.3), colour="purple") + stat_smooth(method="lm") + 
  geom_text(data=raw.corr, aes(x=40, y=ypos, label=VPDest, size=0.2), colour="red", vjust=1, parse=TRUE, show.legend=F) +
  xlab("VPD") + ylab("Branch Position")


VPDweekplots <- ggdraw() +
  draw_plot(Stemoneweek, 0, 0.5, 0.5, 0.5) +
  draw_plot(VPDoneweek, 0, 0, 0.5, 0.5) +
  draw_plot(stems.plot.mean, 0.5, 0.5, 0.5, 0.5) +
  draw_plot(VPD.plot, 0.5, 0, 0.5, 0.5)
save_plot("/Users/alesia/Desktop/ZombiePlots/allVPDplot.png", VPDweekplots, base_width = 12, base_height = 5)
print(WigVPD.cor.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/VPDWiggle_plot.png", width=3.2, height=3, units="in", dpi=300)
...
ggsave(file="/Users/alesia/Desktop/ZombiePlots/RHWiggle_plot.png", width=3.2, height=3, units="in", dpi=300)
...
WigAirT.cor.plot <- ggplot(unique(allregs[,c("timestamp", "air.temp", "mean.dif.all")]), aes(x=air.temp, y=mean.dif.all)) + theme_bw() + theme(legend.position="none") +
  ...
  geom_text(data=raw.corr, aes(x=25, y=0.6, label=AirTest, size=0.2), colour="red", vjust=1, parse=TRUE, show.legend=F) +
...
ggsave(file="/Users/alesia/Desktop/ZombiePlots/AirTWiggle_plot.png", width=3.2, height=3, units="in", dpi=300)
...
ggsave(file="/Users/alesia/Desktop/ZombiePlots/StemWPWiggle_plot.png", width=3.2, height=3, units="in", dpi=300)
...
WigVPDLD.cor.plot <- ggplot(unique(allregs[,c("timestamp", "VPD", "mean.dif.LD", "LDStatus")]), aes(x=VPD, y=mean.dif.LD, group=LDStatus, colour=LDStatus)) + theme_bw() + 
  geom_point(alpha=0.5) + stat_smooth(method="lm") + 
  xlab("VPD") + ylab("Branch Position") + 
  geom_text(data=raw.corr.LD, aes(x=40, y=ypos, label=VPDest), size=3, vjust=1, parse=TRUE, show.legend=F) +
...
WigRHLD.cor.plot <- ggplot(unique(allregs[,c("timestamp", "RH", "mean.dif.LD", "LDStatus")]), aes(x=RH, y=mean.dif.LD, group=LDStatus, colour=LDStatus)) + theme_bw() + 
  geom_point(alpha=0.5) + stat_smooth(method="lm") + 
  xlab("Relative Humidity (%)") + ylab("Branch Position") + 
  geom_text(data=raw.corr.LD, aes(x=25, y=ypos, label=RHest), size=3, vjust=1, parse=TRUE, show.legend=F) +
  labs(colour='Live/Dead') + scale_color_manual(labels = c("Dead", "Live"), values=c("#a65628", "#4daf4a")) +
  theme(legend.key.size = unit(.8, units="lines"),
        legend.position = c(0.75,0.15))
WigRHLD.cor.plot
...
AirTLD.cor.plot <- ggplot(unique(allregs[,c("timestamp", "air.temp", "mean.dif.LD", "LDStatus")]), aes(x=air.temp, y=mean.dif.LD, group=LDStatus, colour=LDStatus)) + theme_bw() + 
  ...
  xlab(expression("Air Temperature " ( degree*C))) + ylab("Branch Position") + 
  geom_text(data=raw.corr.LD, aes(x=25, y=ypos, label=AirTest, size=0.2), vjust=1, parse=TRUE, show.legend=F) +
  labs(colour='Live/Dead') + scale_color_manual(labels = c("Dead", "Live"), values=c("#a65628", "#4daf4a")) +
  theme(legend.key.size = unit(.8, units="lines"),
...
print(WigVPDLD.cor.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/VPDWiggleLD_plot.png", width=3.2, height=3, units="in", dpi=300)
print(WigRHLD.cor.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/RHWiggleLD_plot.png", width=3.2, height=3, units="in", dpi=300)
print(AirTLD.cor.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/AirTWiggleLD_plot.png", width=3.2, height=3, units="in", dpi=300)
print(WigStemWPLD.cor.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/StemWPWiggleLD_plot.png", width=3.2, height=3, units="in", dpi=300)
print(ShadeWiggle.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/ShadeWiggle_plot.png", width=3.2, height=3, units="in", dpi=300)
print(ShadeLD.plot)
ggsave(file="/Users/alesia/Desktop/ZombiePlots/ShadeWiggleLD_plot.png", width=3.2, height=3, units="in", dpi=300)
