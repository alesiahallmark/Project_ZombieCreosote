...
stems.plot.mean <- ggplot(unique(Wigglesub[,c("timestamp", "LDStatus", "StemID", "mean.dif", "mean.dif.all", "mean.dif.LD", "min.stem")]), aes(x=timestamp, y=mean.dif, group=StemID)) +
  theme_bw(base_size=20) + 
  geom_point(colour="light gray") + geom_line(colour="light gray") + 
  geom_point(aes(y=mean.dif.LD, colour=LDStatus), alpha=0.32, size=1) + 
  scale_color_manual(labels = c("Dead", "Live"), values=c("#a65628", "#4daf4a")) +
  geom_point(colour="black", alpha=0.4, size=1, aes(y=mean.dif.all)) + 
  geom_line(colour="black", alpha=0.4, aes(y=mean.dif.all)) +theme(legend.position="none") +
  ylab("Branch Position") + xlab("Time") +
...
allregs <- merge(allregs, Wigglesub[,c("timestamp", "date", "StemID", "LDStatus", "mean.dif", "delta.stem", "min.stem", "max.stem", "minstem.time", "maxstem.amtime", "mean.dif.all", "mean.dif.LD")], by="timestamp", all=T)
...
### All-season plots
# Stems
stems.plot.mean <- ggplot(unique(Wigglesub[,c("timestamp", "LDStatus", "StemID", "mean.dif", "mean.dif.all", "mean.dif.LD")]), aes(x=timestamp, y=mean.dif, group=StemID)) +
  theme_bw(base_size=24) + 
  geom_point(colour="light gray") + geom_line(colour="light gray") + 
  geom_point(aes(y=mean.dif.LD, colour=LDStatus), alpha=0.32, size=1) + 
  scale_color_manual(labels = c("Dead", "Live"), values=c("#a65628", "#4daf4a")) +
  geom_point(colour="black", alpha=0.4, size=1, aes(y=mean.dif.all)) + 
  geom_line(colour="black", alpha=0.4, aes(y=mean.dif.all)) +
...
ccf(allregs$mean.dif.LD[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], allregs$RH[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], lag.max=3000, type="correlation", na.action = na.pass) 
ccf(allregs$mean.dif.LD[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], allregs$air.temp[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], lag.max=3000, type="correlation", na.action = na.pass) 
ccf(allregs$mean.dif.LD[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], allregs$VPD[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], lag.max=3000, type="correlation", na.action = na.pass) 
ccf(allregs$mean.dif.LD[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], allregs$StemWPsm[allregs$LDStatus=="L" & hour(allregs$timestamp) < 10], lag.max=3000, type="correlation", na.action = na.pass) 

plot(allregs$RH, allregs$mean.dif.all) 

ccf(ccr.all$mean.dif.all, ccr.all$StemWPsm, lag.max=6, type="correlation", na.action = na.pass)$acf[1,,1]
...
ccrLD <- cbind("LD", melt(cbind(ccr.LD$timestamp, data.frame(rollapply(ccr.LD, width=24*5, function(x) ccf(as.numeric(x[,"L"]), as.numeric(x[,"D"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.LD$timestamp"))
colnames(ccrLD) <- c("data.comp", "timestamp", "lag", "acf")

# aggregate by day
ccrLD$date <- as.Date(ccrLD$timestamp)
ccrLD$lag <- as.numeric(as.factor(ccrLD$lag)) - 6
ccrLD <- ddply(ccrLD[,c("date", "data.comp", "lag", "acf")], c("date", "data.comp", "lag"), function(x)
  data.frame(ccr.daily = mean(x$acf, na.rm=T)))

library(scales)
ccr.heatmap <- ggplot(ccrLD, aes(x=date, y=lag, fill=abs(ccr.daily))) + 
  geom_raster(interpolate=T) + facet_grid(data.comp~.) + geom_hline(yintercept=0) +
...
# Create a rolling 5-day window to find the maximum cross-correlation throughout the year
ccr.all <- unique(allregs[,c("timestamp", "mean.dif.all", "PAR", "RH", "air.temp", "VPD", "StemWPsm", "FC", "GPP", "RE", "LE")])

ccrPAR <- cbind("PAR", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"PAR"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrPAR) <- c("data.comp", "timestamp", "lag", "acf")

ccrRH <- cbind("RH", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"RH"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrRH) <- c("data.comp", "timestamp", "lag", "acf")

ccrTemp <- cbind("Temp", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"air.temp"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrTemp) <- c("data.comp", "timestamp", "lag", "acf")

ccrVPD <- cbind("VPD", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"VPD"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrVPD) <- c("data.comp", "timestamp", "lag", "acf")

ccrStemWP <- cbind("StemWP", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"StemWPsm"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrStemWP) <- c("data.comp", "timestamp", "lag", "acf")

ccrFC <- cbind("FC", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"FC"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrFC) <- c("data.comp", "timestamp", "lag", "acf")

ccrGPP <- cbind("GPP", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"GPP"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrGPP) <- c("data.comp", "timestamp", "lag", "acf")

ccrLE <- cbind("LE", melt(cbind(ccr.all$timestamp, data.frame(rollapply(ccr.all, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.all"]), as.numeric(x[,"LE"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.all$timestamp"))
colnames(ccrLE) <- c("data.comp", "timestamp", "lag", "acf")

ccr.sofar <- rbind(ccrPAR, ccrRH, ccrTemp, ccrVPD, ccrStemWP)

# aggregate by day
ccr.sofar$date <- as.Date(ccr.sofar$timestamp)
ccr.sofar$lag <- as.numeric(as.factor(ccr.sofar$lag)) - 7
ccr.sofar <- ddply(ccr.sofar[,c("date", "data.comp", "lag", "acf")], c("date", "data.comp", "lag"), function(x)
  data.frame(ccr.daily = mean(x$acf, na.rm=T)))

ccr.heatmap <- ggplot(ccr.sofar, aes(x=date, y=lag, fill=abs(ccr.daily))) + 
  geom_raster(interpolate=T) + facet_grid(data.comp~.) + geom_hline(yintercept=0) +
  scale_fill_gradientn(
    colours = c("#ffffff", "#fff7f3", "#7a0177", "#49006a"), 
    values = rescale(c(0, 0.4, 0.8, 1))) +
  ggtitle("All branch average")
ccr.heatmap


# Cross-correlations with only live branches
ccr.live <- unique(allregs[allregs$LDStatus=="L",c("timestamp", "mean.dif.LD", "PAR", "RH", "air.temp", "VPD", "StemWPsm", "FC", "GPP", "RE", "LE")])

ccrPAR.live <- cbind("PAR", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"PAR"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrPAR.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrRH.live <- cbind("RH", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"RH"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrRH.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrTemp.live <- cbind("Temp", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"air.temp"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrTemp.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrVPD.live <- cbind("VPD", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"VPD"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrVPD.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrStemWP.live <- cbind("StemWP", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"StemWPsm"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrStemWP.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrFC.live <- cbind("FC", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"FC"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrFC.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrGPP.live <- cbind("GPP", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"GPP"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrGPP.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrRE.live <- cbind("RE", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"RE"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrRE.live) <- c("data.comp", "timestamp", "lag", "acf")

ccrLE.live <- cbind("LE", melt(cbind(ccr.live$timestamp, data.frame(rollapply(ccr.live, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"LE"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.live$timestamp"))
colnames(ccrLE.live) <- c("data.comp", "timestamp", "lag", "acf")

ccr.sofar.live <- rbind(ccrPAR.live, ccrRH.live, ccrTemp.live, ccrVPD.live, ccrStemWP.live, ccrLE.live)
ccr.sofar.live$date <- as.Date(ccr.sofar.live$timestamp)
ccr.sofar.live$lag <- as.numeric(as.factor(ccr.sofar.live$lag)) - 6
ccr.sofar.live <- ddply(ccr.sofar.live[,c("date", "data.comp", "lag", "acf")], c("date", "data.comp", "lag"), function(x)
  data.frame(ccr.daily = mean(x$acf, na.rm=T)))

ccr.live.heatmap <- ggplot(ccr.sofar.live, aes(x=date, y=lag, fill=abs(ccr.daily))) + 
  geom_raster(interpolate=T) + facet_grid(data.comp~.) + geom_hline(yintercept=0) +
  scale_fill_gradientn(
    colours = c("#ffffff", "#fff7f3", "#7a0177", "#49006a"), 
    values = rescale(c(0, 0.4, 0.75, 1))) +
  ggtitle("Live branches") + theme()
ccr.live.heatmap


# Cross-correlations with only dead branches
ccr.dead <- unique(allregs[allregs$LDStatus=="D",c("timestamp", "mean.dif.LD", "PAR", "RH", "air.temp", "VPD", "StemWPsm", "FC", "GPP", "RE", "LE")])

ccrPAR.dead <- cbind("PAR", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"PAR"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrPAR.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrRH.dead <- cbind("RH", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"RH"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrRH.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrTemp.dead <- cbind("Temp", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"air.temp"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrTemp.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrVPD.dead <- cbind("VPD", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"VPD"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrVPD.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrStemWP.dead <- cbind("StemWP", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"StemWPsm"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrStemWP.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrFC.dead <- cbind("FC", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"FC"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrFC.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrGPP.dead <- cbind("GPP", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"GPP"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrGPP.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrRE.dead <- cbind("RE", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"RE"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrGPP.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccrLE.dead <- cbind("LE", melt(cbind(ccr.dead$timestamp, data.frame(rollapply(ccr.dead, width=24*5, function(x) ccf(as.numeric(x[,"mean.dif.LD"]), as.numeric(x[,"LE"]), lag.max=5, type="correlation", na.action = na.pass, plot=F)$acf[,,1], by.column=F, partial=T))), id.vars="ccr.dead$timestamp"))
colnames(ccrLE.dead) <- c("data.comp", "timestamp", "lag", "acf")

ccr.sofar.dead <- rbind(ccrPAR.dead, ccrRH.dead, ccrTemp.dead, ccrVPD.dead, ccrStemWP.dead, ccrLE.dead)
ccr.sofar.dead$date <- as.Date(ccr.sofar.dead$timestamp)
ccr.sofar.dead$lag <- as.numeric(as.factor(ccr.sofar.dead$lag)) - 6
ccr.sofar.dead <- ddply(ccr.sofar.dead[,c("date", "data.comp", "lag", "acf")], c("date", "data.comp", "lag"), function(x)
  data.frame(ccr.daily = mean(x$acf, na.rm=T)))

ccr.dead.heatmap <- ggplot(ccr.sofar.dead, aes(x=date, y=lag, fill=abs(ccr.daily))) + 
  geom_raster(interpolate=T) + facet_grid(data.comp~.) + geom_hline(yintercept=0) +
  scale_fill_gradientn(
    colours = c("#ffffff", "#fff7f3", "#7a0177", "#49006a"), 
    values = rescale(c(0, 0.4, 0.75, 1))) + ylim(-4,4) +
  ggtitle("Dead branches") + theme()
ccr.dead.heatmap


grid.arrange(ccr.live.heatmap, ccr.dead.heatmap, ncol=1)
